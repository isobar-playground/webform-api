...
.PHONY: database
database:
	docker-compose run -u 0 mysql wget $(S3_DATABASE_URL) -O /docker-entrypoint-initdb.d/init.sql

## files : download initial media files dump from S3
.PHONY: files
files:
	wget $(S3_FILES_URL) -O files.zip
	mkdir -p drupal/web/sites/default/files
	unzip -o files.zip -d drupal/web/sites/default/files
	rm -rf files.zip
